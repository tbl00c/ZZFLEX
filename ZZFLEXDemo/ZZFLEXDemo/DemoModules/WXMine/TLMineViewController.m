...
- (void)loadMenus
{
    ...
    self.addCell(@"TLMineHeaderCell").toSection(TLMineSectionTagUserInfo).withDataModel(user).selectedAction(^ (id model) {
        @strongify(self);
    ...
    self.addCell(mineMenuCellName).toSection(TLMineSectionTagWallet).withDataModel(wallet);
    ...
    self.addCell(mineMenuCellName).toSection(TLMineSectionTagFounction).withDataModel(collect);
    TLMenuItem *album = createMenuItem(@"mine_album", LOCSTR(@"相册"));
    self.addCell(mineMenuCellName).toSection(TLMineSectionTagFounction).withDataModel(album);
    TLMenuItem *card = createMenuItem(@"mine_card", LOCSTR(@"卡包"));
    self.addCell(mineMenuCellName).toSection(TLMineSectionTagFounction).withDataModel(card);
    TLMenuItem *expression = createMenuItem(@"mine_expression", LOCSTR(@"表情"));
    [expression setBadge:@"NEW"];
    self.addCell(mineMenuCellName).toSection(TLMineSectionTagFounction).withDataModel(expression);
    ...
    self.addCell(mineMenuCellName).toSection(TLMineSectionTagSetting).withDataModel(setting);
